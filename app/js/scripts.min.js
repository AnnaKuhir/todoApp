...
const renderItemList=(list)=>{itemsContainer.innerHTML='';itemsContainer.innerHTML=`
	...
	`}
const fillItemTemplate=(item,id)=>{const status=StatusEnam[item.status];const template=`
	<div class="js-item">
...
const onDeleteButtonClick=(event)=>{debugger;const item=event.target.id;const index=item.split('-').pop();itemList.splice(index,1)
renderItemList(itemList);initializeEventListeners();}
const onDoneButtonClick=(event)=>{const item=event.target.id;const index=item.split('-').pop();}

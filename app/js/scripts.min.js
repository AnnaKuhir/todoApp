...
import{initTodos,createTodo,TODO_STATUS,getTodos,getAllTodos,getTodoById,returnTodos,addTodo,deteleTodo,editTodo,setStatus,setStatusToAll,deteleAll,clearContainer}from'./entity.js';import{renderCreateForm,renderSearchForm,renderTodoList,renderDropboxContent,}from'./templates.js';import{search,sortByTitle,sortByStatus}from'./actions.js'
import{validateCreateForm,validateSearchForm}from'./validationForm.js'
...
form.reset();return validateCreateForm();}
const onSearchTodoClick=(event)=>{event.preventDefault();const form=event.target.parentNode;const input=form[0].value;const filteredList=search(getTodos(),input);renderWithTimeout(itemContainer,filteredList);if(filteredList.length===0){clearContainer(dropboxContainer);}
form.reset();return validateSearchForm();}

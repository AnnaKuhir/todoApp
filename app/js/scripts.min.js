...
const onCreateTodoClick=(event)=>{event.preventDefault();const form=event.target.parentNode;const title=form[0].value;const description=form[1].value;const todo=createTodo(title,description,getAllTodos().length);addTodo(todo);renderTodoList(itemContainer,getTodos());initControlTodoListeners();form.reset();}
const onSearchTodoClick=(event)=>{event.preventDefault();const form=event.target.parentNode;const input=form[0].value;const filteredList=search(getTodos(),input);renderTodoList(itemContainer,filteredList);initControlTodoListeners();initReturnAllAfterSearchControl();if(filteredList.length===0){clearContainer(dropboxContainer);}
form.reset();}
const onSortByTitleClick=()=>{const sortedList=sortByTitle(getTodos());renderTodoList(itemContainer,sortedList)
initControlTodoListeners();}
const onSortByStatusClick=()=>{const sortedList=sortByStatus(getTodos());renderTodoList(itemContainer,sortedList);initControlTodoListeners();}
const onRemoveAllClick=()=>{deteleAll();renderTodoList(itemContainer,getTodos());clearContainer(dropboxContainer);}
...
renderTodoList(itemContainer,getAllTodos());initControlTodoListeners();}
...
renderTodoList(itemContainer,getTodos());initControlTodoListeners();}
const onDeleteButtonClick=(event)=>{const id=+event.target.parentNode.id;deteleTodo(id);renderTodoList(itemContainer,getTodos());initControlTodoListeners();if(getTodos().length===0){clearContainer(dropboxContainer);}}
const onDoneTodoClick=(event)=>{const id=+event.target.parentNode.id;setStatus(id,TODO_STATUS.done);renderTodoList(itemContainer,getTodos());initControlTodoListeners();}
...
debugger
renderTodoList(itemContainer,getTodos());initControlTodoListeners();}
const onReturnAllTodosClick=()=>{const list=returnTodos();renderTodoList(itemContainer,list);initControlTodoListeners();renderDropboxContent(dropboxContainer);initDropboxListeners();}
const onCloseFormClick=()=>{clearContainer(formContainer)}
const onEditTodoClick=(event)=>{debugger;const id=+event.target.parentNode.id;const todo=getTodoById(id);todo.isEdited=true;renderTodoList(itemContainer,getTodos());initControlTodoListeners();}
...
init();export{onAddNewTodoClick,onSearchByTodoClick,onCreateTodoClick,onSearchTodoClick,onSortByTitleClick,onSortByStatusClick,onRemoveAllClick,onHoldToAllStatusClick,onDoneToAllStatusClick,onDeleteButtonClick,onDoneTodoClick,onHoldTodoClick,onReturnAllTodosClick,onCloseFormClick,onEditTodoClick}
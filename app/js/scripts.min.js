...
import{todoList,createTodo,TODO_STATUS,getTodos,addTodo,deteleTodo,editTodo,setStatus,setStatusToAll,deteleAll}from'./entity.js';import{renderCreateForm,renderSearchForm,renderTodoList,renderDropboxContent,clearContainer}from'./templates.js';import{search,sortByTitle,sortByStatus}from'./actions.js'
import{initInitialButtonsListeners,initFormButtonListeners,initDropboxListeners}from'./listeners.js'
const itemContainer=document.querySelector('.js-itemContainer');const dropboxContainer=document.querySelector('.dropbox-container');const onAddNewTodoClick=()=>{const formContainer=document.querySelector('.js-formContainer');if(formContainer){renderCreateForm(formContainer);initFormButtonListeners();}}
...
const onSortByTitleClick=()=>{const sortedList=sortByTitle(todoList);renderTodoList(itemContainer,sortedList)}
const onSortByStatusClick=()=>{const sortedList=sortByStatus(todoList);renderTodoList(itemContainer,sortedList)}
const onRemoveAllClick=()=>{deteleAll();renderTodoList(itemContainer,todoList);clearContainer(dropboxContainer)}
...
init();export{onAddNewTodoClick,onSearchByTodoClick,onCreateTodoClick,onSearchTodoClick,onSortByTitleClick,onSortByStatusClick,onRemoveAllClick,onHoldToAllStatusClick,onDoneToAllStatusClick}